...
def sampleshift(RunConfig):
	...
	nx, ny = np.shape(RunConfig.data)  # (size(data))[1]
	nxo = np.shape(RunConfig.outx)[0] # (size(outx))[1]
	nxi = np.shape(RunConfig.inx)[0]  # (size(inx))[1]
	
	if not np.any(RunConfig.degcontr):
		RunConfig.degcontr = 2

	# if degcontr lt 0 then message, "Degree of fit must be ge 0"
	if RunConfig.degcontr < 0:
	...
	# endif 
	if nx != nxi or ny != np.shape(RunConfig.inx)[1]:
		raise ParameterException("inx must be equal in size to data")

	if ny != np.shape(RunConfig.outx)[1]:
	...
	# if RunConfig.fitsample or RunConfig.fitinterp or RunConfig.fitaverage:
