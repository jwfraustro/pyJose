...
def fitbg(RunConfig):
	...
	dims = np.shape(RunConfig.data_im)
	...
	xvals1 = np.arange(RunConfig.x1)
	xvals2 = np.arange(nx - 1 - RunConfig.x2) + RunConfig.x2 + 1
	xvals = [xvals1, xvals2]

	# Subtract bias
	if RunConfig.nobgfit:
		bgim = np.ones((nx, ny)) * np.median(RunConfig.data_im[xvals, :])
		return bgim

	# Prepare for row by row fitting
	RunConfig.yvals = np.arange(ny)
	RunConfig.bgim = RunConfig.data_im
	...
	for i in range(ny):
		RunConfig.datav = RunConfig.data_im[:, i]
		...
		RunConfig.varim[xvals, i] = RunConfig.varv[xvals]
